...
import json
import pyspark
from pyspark.sql.functions import col, collect_list, array_join, struct, desc
...
##### FROM FILES
tedx_dataset_path = "s3://tedx-2026-data-mp1-gp/final_list.csv"
...
## READ THE DETAILS
details_dataset_path = "s3://tedx-2026-data-mp1-gp/details.csv"
...
## READ TAGS DATASET
tags_dataset_path = "s3://tedx-2026-data-mp1-gp/tags.csv"
...
## WATCH_NEXT
watch_next_dataset_path = "s3://tedx-2026-data-mp1-gp/watch_next.csv"
...
watch_next_resolved = watch_next_dataset.join(
    ...
    .select(
        ...
        col("viewedCount").cast("long").alias("viewed_count")
    ) \
    .filter(col("related_public_id").isNotNull()) \
    .orderBy(desc("viewed_count"))
